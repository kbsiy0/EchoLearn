...
- **Phase 2（已完成 2026-04-29，PR #12）**：影片歷史與學習進度恢復（per-video last-played + speed + loop persisted；server-side `video_progress` table；ResumeToast；HomePage 重排序 + 進度條 + 重置按鈕）。Spec 已 archive 至 `openspec/archive/phase2-history-and-resume/`.
