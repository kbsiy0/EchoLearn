# Phase 1b: Segment Streaming — UI Verification Report

- **Date:** 2026-04-26
- **Verdict: PASS**
- **Commit verified:** `9e8cc08`
- **Branch:** `change/phase1b-segment-streaming`
- **Source:** T12 ui-verifier run (full details in [`t12/report.md`](t12/report.md))
...
- [x] Step 1: Home page loads — URL input present, empty history (fresh DB). Screenshot: [`t12/screenshots/golden/00-home.png`](t12/screenshots/golden/00-home.png)
- [x] Step 2: Submit `jNQXAC9IVRw` URL → navigate to `/watch/jNQXAC9IVRw` in **96–109ms** (threshold < 200ms). Screenshot: [`t12/screenshots/golden/01-player-initial.png`](t12/screenshots/golden/01-player-initial.png)
- [x] Step 3: `ProcessingPlaceholder` renders with progress bar and "處理字幕中 (0%)" text. Screenshot: [`t12/screenshots/golden/02-processing-empty.png`](t12/screenshots/golden/02-processing-empty.png)
- [x] Step 4: Processing completes; full `CompletedLayout` mounts — VideoPlayer + subtitle panel ("字幕 (3 句)") + `PlayerControls`. Screenshot: [`t12/screenshots/golden/04-completed-player.png`](t12/screenshots/golden/04-completed-player.png)
- [x] Step 5: Cache-hit path — second navigation immediately shows completed player in **231ms**. Screenshot: [`t12/screenshots/golden/05-cache-hit-completed.png`](t12/screenshots/golden/05-cache-hit-completed.png)
...
- [x] **Failed state UI (VIDEO_UNAVAILABLE):** `h3="處理失敗"`, gray error message "內部錯誤", `回首頁` button visible. Clicking navigates to `/`. Screenshot: [`t12/screenshots/failed/failed-state-full.png`](t12/screenshots/failed/failed-state-full.png). PASS.
...
### 20-minute completion SLO not measured

The Phase 1b spec requires that a 20-minute video completes end-to-end in ≤ 3 minutes at p95. This SLO could not be measured in the current dev environment:

- yt-dlp 2026.03.17 requires a deno/node JS runtime for modern YouTube formats
- Without the runtime, only 3 specific short videos (≤ 7 min) were accessible in this env
- No 20-minute video fixture was available

**What was observed instead:**
- The 19s test video (`jNQXAC9IVRw`) processed end-to-end in ~10.5s (TTFS) — well within the proportional budget
- Backend unit tests (`test_audio_chunking.py`, `test_segmenter.py`, `test_pipeline_streaming.py` × 20 cases) cover the per-chunk streaming pipeline mechanics including carryover and retry logic
- The `test_pipeline_golden.py` integration tests verify the full pipeline against fake Whisper + real DB

**Deferred validation:** Real-world 20-minute completion SLO will be validated post-merge in a fully-keyed env (OPENAI_API_KEY + yt-dlp with JS runtime) running against a longer fixture.

### VIDEO_TOO_LONG screenshot

The `VIDEO_TOO_LONG` error mode uses a substitute `VIDEO_UNAVAILABLE` path in the Playwright run (same error-mode UI path, different error code). `_SAFE_MESSAGES["VIDEO_TOO_LONG"]` is covered by backend unit tests.
...
**PASS** — All Phase 1b invariants hold: immediate navigation (96ms), TTFS fires once (10484ms, finite), VideoPlayer mounts exactly once, cache-hit skips processing, failed-state shows 回首頁 and navigates back; 146 vitest + 275 pytest all green. 20-min completion SLO deferred to a fully-keyed environment per the yt-dlp env limitation.
